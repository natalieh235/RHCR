...
public:
    ...
    CBasicSystem(const BasicGraph& G, CMAPFSolver& solver);
    ~CBasicSystem();
    ...
    void update_paths(const std::vector<CPath*>& MAPF_paths, int max_timestep);
    void update_paths(const std::vector<CPath>& MAPF_paths, int max_timestep);
    void update_initial_paths(vector<CPath>& initial_paths) const;
    void update_initial_constraints(list< tuple<int, int, int> >& initial_constraints) const;
    
	void add_partial_priorities(const vector<CPath>& initial_paths, PriorityGraph& initial_priorities) const;
