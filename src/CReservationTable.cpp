...
void ContinuousReservationTable::build(
    const vector<CPath *> &paths,
...
vector<CInterval> ContinuousReservationTable::getSafeIntervals(int loc, double t_start, double t_end)
{
    ...
    for (auto &interval : reserved)
    {
        ...
        if (r_end <= t_start || r_start > t_end)
            continue;
...
void ContinuousReservationTable::insertPath(const CPath &path)
{
    ...
    for (int i = 0; i < path.size() - 1; i++)
    {
        ...
        std::cout << "start time " << start_time << std::endl; // when last movement ended
        ...
        for (int j = 0; j < profile.length; j++)
        {
            ...
            if (j == 0)
            {
                ...
                // the first cell includes the turning and waiting time
                end_interval = start_interval + std::get<1>(profile.entries[j]) + turn_time + wait_time;
            }
            else
            {
                // otherwise turning and waiting happens before you start occupying the cell
            ...
            if (j == profile.length - 1)
            {
                ...
                // starting res time of the last cell in the segment
                prev_time = std::get<0>(profile.entries[j + 1]);
            }
        }
    }
}