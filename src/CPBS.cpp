...
// void CPBS::update_paths(CPBSNode* curr)
// {
...
// void CPBS::copy_conflicts(const list<Conflict>& conflicts,
...
// }
// void CPBS::copy_conflicts(const list<Conflict>& conflicts, list<Conflict>& copy, int excluded_agent)
// {
//     clock_t t = clock();
//     for (auto conflict : conflicts)
//     {
...
// void CPBS::find_conflicts(list<Conflict>& conflicts, int a1, int a2)
// {
//     clock_t t = clock();
...
// void CPBS::find_conflicts(list<Conflict>& conflicts)
// {
...
// void CPBS::find_conflicts(list<Conflict>& new_conflicts, int new_agent)
// {
...
// void CPBS::find_conflicts(const list<Conflict>& old_conflicts, list<Conflict>& new_conflicts, int new_agent)
// {
//     // Copy from parent
...
// void CPBS::remove_conflicts(list<Conflict>& conflicts, int excluded_agent)
// {
//     clock_t t = clock();
...
// void CPBS::choose_conflict(CPBSNode &node)
// {
//     clock_t t = clock();
...
// double CPBS::get_path_cost(const Path& path) const
// {
//     double cost = 0;
...
// // find_path
// bool CPBS::find_path(CPBSNode* node, int agent)
// {
//     Path path;
//     double path_cost;
...
//     clock_t t = std::clock();
// 	rt.copy(initial_rt);
//     // std::cout << "CPBS find path" << initial_constraints.size() << std::endl;
...
//     if (screen == 2)
//         std::cout << "      CPBS find_path: running SIPP for agent " << agent << std::endl;
...
// bool CPBS::wait_at_start(const Path& path, int start_location, int timestep)
// {
//     for (auto& step : path)
//     {
...
// void CPBS::find_replan_agents(CPBSNode* node, const list<Conflict>& conflicts,
...
// bool CPBS::find_consistent_paths(CPBSNode* node, int agent)
// {
//     if (screen == 2)
//         std::cout << "  CPBS: Finding consistent paths for agent " << agent << " in node with priorities" << node->priority.first << " -> " << node->priority.second << std::endl;
//     clock_t t = clock();
...
//     while (!replan.empty()) {
//         if (screen == 2) {
//             std::cout << "  CPBS must replan agents: ";
//             for (auto a : replan)
//                 std::cout << a << ", ";
//             std::cout << std::endl;
//         }
...
//         if (screen == 2)
//             std::cout << "  CPBS consistent paths: looking for path for agent " << a << std::endl;
//         if (!find_path(node, a)) {
//             runtime_find_consistent_paths += (double)(std::clock() - t) / CLOCKS_PER_SEC;
//             return false;
//         }
...
//         if (screen == 2) {
//             std::cout << "  after new plan, CPBS must replan agents: ";
//             for (auto a : replan)
//                 std::cout << a << ", ";
//             std::cout << std::endl;
//         }    
...
//     // std::cout << "  find_consistent_paths: finished, replan is empty" << std::endl;
//     runtime_find_consistent_paths += (double)(std::clock() - t) / CLOCKS_PER_SEC;
//     if (screen == 2)
...
// bool CPBS::validate_consistence(const list<Conflict>& conflicts, const PriorityGraph &G)
// {
//     for (auto conflict : conflicts)
//     {
//         int a1 = std::get<0>(conflict);
...
// bool CPBS::generate_child(CPBSNode* node, CPBSNode* parent)
// {

//     if (screen == 2)
//         std::cout << "  CPBS: Generating CPBS child node" << std::endl;
// 	node->parent = parent;
...
//         if (screen == 2)
//             std::cout << "      CPBS: adding priority " << node->priority.first << " -> " << node->priority.second << std::endl;
...
//             if (screen == 2)
//                 std::cout << "      CPBS: failed to find consistent paths" << std::endl;
//             return false;
//         }
//             // return false;
//     }
...
//     if (screen == 2)
//         std::cout << "      CPBS generated child node successfully" << std::endl;
...
// bool CPBS::generate_root_node()
// {
//     std::cout << "CPBS: Generating CPBS root node" << std::endl;
//     clock_t time = std::clock();
// 	dummy_start = new CPBSNode();
...
//         if (screen == 2)
//             std::cout << " CPBS: generating root node, running SIPP for " << i << std::endl;

//         path = path_planner.run(G, starts[i], goal_locations[i], rt);
  
// 		runtime_plan_paths += (double)(std::clock() - t) / CLOCKS_PER_SEC;

//         // check CSIPP sets path cost
//         path_cost = path_planner.path_cost;
...
// void CPBS::push_node(CPBSNode* node)
// {
//     dfs.push_back(node);
//     allNodes_table.push_back(node);
// }

// CPBSNode* CPBS::pop_node()
// {
//     CPBSNode* node = dfs.back();
//     dfs.pop_back();
//     return node;
// }

// void CPBS::update_best_node(CPBSNode* node)
// {
...
// bool CPBS::run_continuous(const vector<CState>& starts,
...
// 		if(screen == 2)
// 			std::cout << "\nCPBS: Expand Node " << curr->time_generated << " ( cost = " << curr->f_val << " , #conflicts = " <<
...
//                     if (screen == 2)
//                         std::cout << "  CPBS: successfully generated child node" << std::endl;
//                     break;
//                 }
//             }
// 		    else {
...
//         if (!solution_found) {   
//             if (screen == 2)
//                 std::cout << "  CPBS: solution not found, adding child to dfs" << std::endl;
//             if (n[0] != nullptr && n[1] != nullptr) {
//                 // std::cout << "      adding both nodes" << std::endl;
...
//     if (screen == 2)
//         std::cout << "  CPBS: DFS DONE" << std::endl;
...
// void CPBS::resolve_conflict(const Conflict& conflict, CPBSNode* n1, CPBSNode* n2)
// {
//     int a1, a2, v1, v2, t;
//     std::tie(a1, a2, v1, v2, t) = conflict;
...
// CPBS::CPBS(const BasicGraph& G, SingleAgentSolver& path_planner) : MAPFSolver(G, path_planner),
//         lazyPriority(false), best_node(nullptr) {}


// inline void CPBS::release_closed_list()
// {
...
// void CPBS::update_focal_list()
// {
// 	/*CPBSNode* open_head = open_list.top();
...
// void CPBS::update_CAT(int ex_ag)
// {
//     size_t makespan = 0;
...
// void CPBS::save_results(const std::string &fileName, const std::string &instanceName) const
// {\
// 	std::ofstream stats;
...
// void CPBS::print_conflicts(const CPBSNode &curr)
// {
...
// void CPBS::save_search_tree(const std::string &fname) const
// {
//     std::ofstream output;
...
// void CPBS::save_constraints_in_goal_node(const std::string &fileName) const
// {
// 	best_node->priorities.save_as_digraph(fileName );
// }

// void CPBS::get_solution()
// {
