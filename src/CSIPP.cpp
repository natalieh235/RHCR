...
std::tuple<bool, CPath> CSIPP::update_goals(CSIPPNode *curr, const vector<pair<int, int>> &goal_locations)
{
    ...
    // cout << "       csipp: updating goals, " << curr->state.location << " , " << curr->goal_id << endl;
        ...
        curr->state.timestep >= goal_locations[curr->goal_id].second) // reach the goal location after its release time
    {
        ...
        {
            cout << "       CSIPP: reached goal location " << curr->goal_id -1<< ", " << curr->state << endl;
            // return path;
            // return {true, Path()};
            return {true, updatePath(curr)};
        }

        CSIPPNode *new_node = new CSIPPNode(curr->state, curr->g_val, curr->h_val, curr->interval, curr->parent, curr->conflicts, goal_locations[curr->goal_id], 0);
        // def need this line
        new_node->goal_id++;
                            ...
                            ContinuousReservationTable &rt)
{
    ...
    cout << "   running csipp: start: " << start << ", goal: " << goal_locations[0].first << endl;
    num_expanded = 0;
    num_generated = 0;
    runtime = 0;
    clock_t t = std::clock();
    ...
    std::cout << "  csipp: starting search" << std::endl;
